...
<Steps>
  <Step title="Install the extension">
    In Phantom, open the Extensions pane in the sidebar (or Settings → Extensions) and press **Install** next to Modus Vivendi Deuteranopia.
  </Step>
  ...
  <Step title="Pick Modus Vivendi Deuteranopia">
...
The whole extension is `themes/modus-vivendi-deuteranopia.conf`. It is a Ghostty theme file: one `key = value` per line, `#` for comments, and colour keys only. This one sets 22 values.
...
Settings → Extensions has a **Reveal in Finder** button. Copy `themes/modus-vivendi-deuteranopia.conf`, change what you want, and drop the file into your own themes folder, where Settings → Appearance lists it under **Custom Themes**.
...
Modus Vivendi Deuteranopia is a terminal palette. It sets the sixteen ANSI colours, the background and the foreground, and it stops there, so the window chrome and the macOS icon keep the colours the app gives them and follow the window's own appearance.
