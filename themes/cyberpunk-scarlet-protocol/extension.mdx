...
<ThemePreview title="Cyberpunk Scarlet Protocol" background="#101116" foreground="#e41951" cursor="#76ff9f" selection="#c1deff" ansi="#101116, #ff0051, #01dc84, #faf945, #0271b6, #c930c7, #00c5c7, #c7c7c7, #686868, #ff6e67, #60fa68, #fffc67, #6871ff, #bd35ec, #60fdff, #ffffff" cursorText="#a6a6a6" selectionText="#000000" />
...
<Steps>
  <Step title="Install the extension">
    In Phantom, open the Extensions pane in the sidebar (or Settings → Extensions) and press **Install** next to Cyberpunk Scarlet Protocol.
  </Step>
  ...
  <Step title="Pick Cyberpunk Scarlet Protocol">
...
The whole extension is `themes/cyberpunk-scarlet-protocol.conf`. It is a Ghostty theme file: one `key = value` per line, `#` for comments, and colour keys only. This one sets 22 values.
...
Settings → Extensions has a **Reveal in Finder** button. Copy `themes/cyberpunk-scarlet-protocol.conf`, change what you want, and drop the file into your own themes folder, where Settings → Appearance lists it under **Custom Themes**.
...
Cyberpunk Scarlet Protocol is a terminal palette. It sets the sixteen ANSI colours, the background and the foreground, and it stops there, so the window chrome and the macOS icon keep the colours the app gives them and follow the window's own appearance.

</Details>

<Details summary="Text printed in ANSI black is hard to read">

ANSI Black is `#101116`, which sits 1.00:1 against the background. That is the palette's own value. A program that prints in ANSI 0 on the default background is close to invisible here, and the theme keeps the value rather than inventing a different one.
